...
class Request:
  ...
  def __init__(self, query, data=None):
    ...
    # if data is not None:
    #   print( 'cached    ', self.query.stringify() )
  ...
  def get_request(self):
    def https_request(retry=0):
      ...
      try:
        req = requests.post(
          self.v2_endpoint,
          headers={'Authorization': self.token.auth},
    ...
    def get_path(node):
      ...
      if any(
        [field.get('fields') or field.get('args') for field in node.get('fields')]
      ):
        child_fields = [get_path(field) for field in node.get('fields')]
        child_fields = (
          child_fields[0] if isinstance(child_fields[0], list) else child_fields
        )
    ...
    if (
      ...
      and isfunction(self.query.paginator)
      and isfunction(self.query.pagination_handler)
    ):
      if any([self.query.paginator(sub_body, self.query) for sub_body in body]):
        self.query.pagination_handler(body, self.get_request())
