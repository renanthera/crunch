...
tmpc = wcl.query.MasterData( '5678' )
tmpd = wcl.query.PointsSpentThisHour()
...
class Test:
  ...
  def update( self, *params ):
    ...
    assert all([type(pair[0]) is type(pair[1]) or pair[1] is None for pair in zip(self.params, params)]), 'Types of params are not matching'
    assert params != self.params, 'Params are unchanged'

    # There's probably a better way to update the tuple more-in-place than this,
...
class TestB( Test ):
  def __init__( self, *params ):
    super().__init__( self.__class__, *params )

u = TestB( 'c', 'd', 3, 4 ).make()
print( u.params )
u.update( 'e', 'f', 5, 6 )
print( u.params )
u.update( 'q', None, None, 61 )
print( u.params )
