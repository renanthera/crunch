...
class Stagger:
        ...
        match (event.get('type')):
            case 'damage':
                if not isID(event, 124255):
                    ...
                    amount = self.fmt((event.get('amount', 0) + event.get('absorbed', 0)) * self.stagger_p(event))
                    pool = self.fmt(self.pool)
                    print(f'{self.index:<4} {bcolors.GREEN}a{bcolors.ENDC}: {timestamp:<15} {amount:<10} {pool:<10}')
                if isID(event, 124255):
                    timestamp = t(event.get('timestamp'))
                    amount = self.fmt(event.get('unmitigatedAmount'))
                    tick = self.fmt(math.ceil(self.tick + self.tick_adjust))
                    if amount == tick:
                        print(f'{self.index:<4} {bcolors.RED}t{bcolors.ENDC}: {timestamp:<15} {amount:<10} {tick:<10}')
                    else:
                        print(f'{self.index:<4} {bcolors.RED}t{bcolors.ENDC}: {bcolors.RED_B}{timestamp:<15} {amount:<10} {tick:<10}{bcolors.ENDC}')
    ...
    def process(self):
        ...
        while self.index < len(self.data):
            ...
            match (event.get('type')):
                case 'damage':
                    self.hit_types[event.get('hitType', None)] = self.hit_types.get(event.get('hitType'), 0) + 1
                    if not isID(event, 124255) and event.get('hitType', 0) not in self.invalid_hit_types:
                        amount = (event.get('amount', 0) + event.get('absorbed', 0)) * self.stagger_p(event)
                        self.pool += (amount)
                        self.remainder = 0
                        ...
                        if dist > 32 and self.reorder:
                            ...
                            for x in range(self.index, -1, -1):
                                if self.data[x].get('type') == 'absorbed':
                                    if self.data[x].get('timestamp') != self.data[self.index].get('timestamp'):
                                        print(f'FAILED TO REORDER AT {self.index}, {x}')
                                        return
                                    print(f'REORDERING {x} WITH {self.index}')
                                    self.data[x], self.data[self.index] = self.data[self.index], self.data[x]
                                    self.index = 0
                                    self.reset()
                                    break
                            continue
