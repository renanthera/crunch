...
def bug_detection( report_codes ):
  ...
  def fight_params_update( self ):
    ...
    clause_2 = f"type in ('damage') and ability.id in ({', '.join(map(str,damage_ids))})"
  ...
  def pta_buff( self, event ):
    ...
    last_stack = previous_application[ 'stack' ] if event.get( 'timestamp' ) - previous_application[ 'timestamp' ] < 20 * 1000 else 0
    self.event_data[ 'pta_stacks' ].update( {
      'stack': event.get( 'stack', last_stack ),
      'timestamp': event.get( 'timestamp' )
    } )
  ...
  def other_buffs( self, event ):
    ...
    if event.get( 'type' ) == 'refreshbuff':
      assert buff_status == True, f'buff refreshed when it was not already applied\n{event}'
      self.event_data[ 'other_buffs' ][ event.get( 'abilityGameID' ) ][ 'up' ] = True
    if event.get( 'type' ) == 'removebuff':
      assert buff_status == True, f'buff removed when it was not already applied\n{event}'
      self.event_data[ 'other_buffs' ][ event.get( 'abilityGameID' ) ][ 'up' ] = False
    self.event_data[ 'other_buffs' ][ event.get( 'abilityGameID' ) ][ 'timestamp' ] = event.get( 'timestamp' )
      ...
      if event_next.get( 'type' ) == 'cast' and event_next.get( 'abilityGameID' ) in cast_ids:
        ...
        if not event_next.get( 'triggered_by_pta' ):
          event_next.update( {
            ...
            'trigger_timestamp': event.get( 'timestamp' )
          } )
        break
  ...
  analyzer = Analyzer(
    ...
    callbacks=[
      ...
      {
        'abilityGameID': damage_ids,
        'type': 'damage',
        'callback': consume_pta
      },
      {
        'abilityGameID': cast_ids,
        'type': 'cast',
        'callback': check_buffs
      },
    ],
    event_data={
      'pta_stacks': {
