...
def fight_params_update(self):
  ...
  clauses = ' or '.join([clause_1, clause_2])
...
def accumulate(self, event):
  ...
  if event.get('abilityGameID') not in blacklist and event.get('sourceID') != event.get(
    'targetID'
  ):
    print(fmt_timestamp(event.get('timestamp') - self.params['startTime']), end='')
    print(
      f' {self.event_data["value"]}+={get_damage(event)}*0.25 ({get_damage(event) * 0.25})'
    )
...
def spender(report_codes):
  Analyzer(
    ...
    callbacks=[
      ...
      {'type': 'damage', 'callback': accumulate},
      {'type': 'damage', 'abilityGameID': 450763, 'callback': p},
    ],
    event_data={'is_accumulating': False, 'value': 0},
