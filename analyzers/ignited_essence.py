...
def ignited_essence(reportCodes):
  def ignited_essence_debuff(event, event_data):
    ...
    match event.get('type'):
      case 'applydebuffstack':
        ...
        ignited_essence.update({source_id: stack})
      case 'applydebuff':
        ignited_essence.update({source_id: 1})
      case 'removedebuff':
        if ignited_essence.get(source_id) != 5:
          print(
            '   ',
            wcl.getPlayerFromID(source_id, params),
            ignited_essence.get(source_id),
          )
  ...
  def emberscars_mark_debuff(event, event_data):
    ...
    match event.get('type'):
      case 'applydebuff':
        emberscars_mark.update({target_id: 1})
      case 'removedebuff':
        emberscars_mark.update({target_id: 0})
  ...
  def devour_essence_cast(_, event_data):
    ...
    for player, status in event_data['emberscars_mark'].items():
      if status:
        event_data['ignited_essence'].update({player: 0})
  ...
  report_code_to_events(
    ...
    {'emberscars_mark': {}, 'ignited_essence': {}, 'slam_count': 1},
    [
      {'abilityGameID': 421858, 'callback': ignited_essence_debuff},
      {'abilityGameID': 421643, 'callback': emberscars_mark_debuff},
      {'abilityGameID': 422277, 'callback': devour_essence_cast},
    ],
  )
