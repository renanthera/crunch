...
def source(analyzer, event):
  ...
  source_amount = (
    event.get('amount', 0) + event.get('absorbed', 0) + event.get('overkill', 0)
  )
  source_timestamp = event.get('timestamp')
  is_crit = event.get('hitType', -1) == 2
  if source_id in analyzer.event_data['events'].keys():
    analyzer.event_data['events'][source_id].append(
      Event(source_id, ability_game_id, source_amount, is_crit, source_timestamp)
    )
  else:
    analyzer.event_data['events'][source_id] = [
      Event(source_id, ability_game_id, source_amount, is_crit, source_timestamp)
    ]
...
def fight_params_update(self):
  ...
  return {'filterExpression': f'{clause_0} and {clause_1}'}
...
def coefficient(report_codes):
  ...
  a = Analyzer(
    ...
    callbacks=[
      {'any': False, 'callback': lambda _, e: print(json.dumps(e, indent=2))},
      {'type': 'damage', 'abilityGameID': [205523, 100780, 185099], 'callback': source},
      {'abilityGameID': 452333, 'callback': final},
    ],
    event_data={
      'events': dict(),
    },
  ...
  coeff = [round(0.25 / a / b * c, 3) for a in d1 for b in d2 for c in m1]
  coeff_2 = [f'{a:.2f}\t{b:.2f}\t{c:.2f}' for a in d1 for b in d2 for c in m1]
  ...
  doublecrits = [
    event for event in flat_events if event.source_is_crit and event.final_is_crit
  ]
  ...
  errors = [
    ...
    if round(event.ratio, 3) not in coeff and event.ratio > 0
  ]
  ...
  print()
  print(
    f'error counts - c: {len(crit_errors)}, nc: {len(noncrit_errors)}, t: {len(errors)}'
  )
  ...
  print(
    f'crit chance by source: { ({k: round(crits_by_source[k] / hits_by_source[k] * 100, 2) for k in crits_by_source.keys()}) }'
  )
  print('hits by ratio:\nratio\tcount\test\t(/woo\t/coale\t*crit)')
  for coeff_pair in sorted(zip(coeff, coeff_2), key=lambda v: v[0]):
    print(
      f'{coeff_pair[0]}\t{coefficient_count[coeff_pair[0]]}\t{round(coefficient_count[coeff_pair[0]] * crit_chance / 100)}\t({coeff_pair[1]})'
    )
