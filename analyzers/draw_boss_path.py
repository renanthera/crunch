...
class Screen:
  ...
  def transform(self, point: Point):
    return Point(
      point.from_source, point.from_target,
...
def init_targets(self, _):
  targets = [entity for entity in wcl.getMasterData( self.params ) if entity.get('gameID') == self.params.get('zzz_boss_id')]
  self.event_data = [
    {
      'id': target.get('id'),
...
def process_movement(self, event):
  ...
  indices = [index for index, value in enumerate(self.event_data) if value.get('id') == key]
  index = indices[0]

  x = event.get('x', 0)
  y = event.get('y', 0)
  point = Point(
    source_in_targets, target_in_targets,
    x, y
  )
...
def draw_path(report_codes, target_id, params):
  ...
  a = Analyzer(
    ...
    callbacks=[
      {
        'type': 'damage',
        'callback': process_movement
      },
      {
        'type': 'encounterstart',
        'callback': init_targets
      },
    ],
  ...
  for index, fight in enumerate(a.data):
    for target in fight['event_data']:
      ...
      ax[index//3, index%3].plot([point.x for point in filtered_points], [point.y for point in filtered_points], linewidth=2)
  plot.show()
